...
def check_if_program_installed(program_name):
    ...
    """
    result = (get_program_status(config.PREREQUISITE_INSTALLATIONS[program_name]) is not None)
...
def check_if_process_is_running(process_name):
    ...
    """
    result = (config.PREREQUISITE_PROCESSES[process_name] in (p.name() for p in psutil.process_iter()) is not None)
